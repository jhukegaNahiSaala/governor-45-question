...
console.log("sorry the new dinner table won’t arrive in time for the dinner so I can only invite two person\n");

while (people_to_Invite.length > 2) {
   let rGuest = people_to_Invite.pop()
   console.log(`sorry ${rGuest} I cant invite you because table wont arrive at time`)
}
for (const iterator of people_to_Invite) {
   console.log(`Hello ${iterator} you are still invited in the dinner`)
}
people_to_Invite.splice(0,people_to_Invite.length)
console.log(people_to_Invite);
